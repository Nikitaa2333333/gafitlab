...
## Instructions
- **STRICT FORBIDDEN TOOL**: You are STRICTLY FORBIDDEN from calling `run_command`, `send_command_input`. Even with `SafeToAutoRun: true` - it is a DIRECT VIOLATION.
...
- **Verification**: Provide the command and STOP. Do not proceed until the user says "done" or "next".
